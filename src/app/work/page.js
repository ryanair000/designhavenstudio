...
export default async function WorkPage({searchParams}){
 ...
 const categorySlug=(value='')=>value.toLowerCase().replaceAll(' ','-').replaceAll('&','').replaceAll('--','-')
 const projects=(data.projects||[])
  .filter((project)=>!project.status||project.status==='published')
  .filter((project)=>!selected||categorySlug(project.category).includes(selected)||project.category===selected)
  .sort((a,b)=>{
   const priority=(project)=>String(project.id||'').startsWith('client-')?0:1
   return priority(a)-priority(b)||(a.sort_order||999)-(b.sort_order||999)
  })
